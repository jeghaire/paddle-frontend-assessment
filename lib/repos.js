export async function getAllReposData(page = 1) {
  const fetchCountries = await fetch(`https://api.github.com/search/repositories?q=created:>2021-08-13&sort=stars&order=desc&page=${page}`,
    {
      headers: {
        'accept': 'application/vnd.github+json',
      }
    })
  const repos = await fetchCountries.json()
  return repos
}
